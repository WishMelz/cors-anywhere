...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    isStaticAsset(location.href, function (err, ok) {
      if (err || !ok) {
        res.writeHead(403, { 'Content-Type': 'text/plain' });
        return res.end('Only static assets are allowed.');
      }
      proxyRequest(req, res, proxy);
    });
  };
...
function isStaticAsset(targetUrl, cb) {
  ...
  var req = mod.request({
    ...
    var type = (res.headers['content-type'] || '').split(';')[0].toLowerCase();
    var ok = [
     // 样式 & 脚本
      'text/css',
      ...
      // 脚本文件
      'application/x-sh', 'application/x-shellscript', 'text/x-sh', 'text/plain', // .sh
      'text/x-python', 'application/x-python', // .py
      'application/x-bat', // .bat
      'application/wasm', // WebAssembly
      'application/x-php', 'text/x-php', // PHP 脚本
  ...
  req.on('error', cb);
  req.on('timeout', () => req.abort() && cb(null, false));
  req.end();
}
